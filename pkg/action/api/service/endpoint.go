...
// Get Endpoint
func (s *serviceServer) GetEndpoint(g *gin.Context) {
	...
	item, err := s.Endpoint.Get(namespace, name)
	if err != nil {
...
// Subscribe Endpoint
func (s *serviceServer) ListEndpoint(g *gin.Context) {
	list, err := s.Endpoint.List(g.Param("namespace"), "")
	if err != nil {
...
func (s *serviceServer) ApplyEndpoint(g *gin.Context) {
	namespace := g.Param("namespace")
	...
	newUnstructuredExtend, isUpdate, err := s.Endpoint.Apply(namespace, name, &service.UnstructuredExtend{Unstructured: _unstructured})
	if err != nil {
...
// Delete Endpoint
func (s *serviceServer) DeleteEndpoint(g *gin.Context) {
	namespace := g.Param("namespace")
	name := g.Param("name")
	if namespace == "" || name == "" {
		common.RequestParametersError(g, fmt.Errorf("params not obtain namespace=%s name=%s", namespace, name))
	...
	err := s.Endpoint.Delete(namespace, name)
	if err != nil {
		common.InternalServerError(g, err, err)
		return
	}
