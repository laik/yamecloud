...
func (s *accessControlServer) ListClusterRoleBind(g *gin.Context) {
	list, err := s.ClusterRoleBinding.List("", "")
	if err != nil {
...
// Get ClusterRoleBind
func (s *accessControlServer) GetClusterRoleBind(g *gin.Context) {
	...
	item, err := s.ClusterRoleBinding.Get(namespace, name)
	if err != nil {
		common.InternalServerError(g, "", err)
		return
	}
...
// Update or Create ClusterRoleBind
func (s *accessControlServer) ApplyClusterRoleBind(g *gin.Context) {
	name := g.Param("name")
	if name == "" {
	...
	newUnstructuredExtend, isUpdate, err := s.ClusterRoleBinding.Apply(namespace, name, &service.UnstructuredExtend{Unstructured: _unstructured})
	if err != nil {
